...
function config(callback){
  if(cache!=undefined){
    callback && callback(undefined, cache);
    return cache;
  }
  ...
  readFile(filename(), function(error, data){
    ...
    try { 
      cache = JSON.parse(String(data));
      read();
    } catch(jsonParsingError) {
      callback && callback(jsonParsingError);
      return;
    }
...
function read(){
  ...
  commander.commands(cache.commands);
  ...
  var rev = cache.revision || cache.version;
  rev && revision(rev);
