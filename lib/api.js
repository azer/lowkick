...
function command(req, res){
  ...
  commander.run(commandName, options, function(error, result){
    ...
    if(!req.body['wait-end-signal'] && !req.body.options['wait-end-signal']){
      respond(res, { 'command': commandName, 'result': result });
    } else {

      (function waitEndSignal(){

        logging.info('Waiting for the end signal...');

        if(result.sandbox.signals.indexOf('END') == -1){
          setTimeout(waitEndSignal, 1000);
        } else {
          respond(res, { 'command': commandName, 'result': result });
        }
