...
function command(req, res){
  ...
  commander.run(commandName, options, function(error, result){
    ...
    var sandbox           = result.sandbox,
        _waitForEndSignal = sandbox && ( req.body['wait-end-signal'] || req.body.options['wait-end-signal'] ),
        _waitForResult    = sandbox && ( req.body['wait-result'] || req.body.options['wait-result'] ),
        
        _respond           = respond.bind(undefined, res, { 'command': commandName, 'result': result }, undefined);
...
function fail(req, res){
  ...
  if(sandboxName){
    sbox = sandbox.get(sandboxName);
    sbox.result = report.genResultSet(options.environ, fail);
    return;
  }
